...
|第2列| 遗传距离，遗传距离是基于遗传连锁关系的，它反映了两个位点之间的重组频率。|
|第3列| 遗传距离相对位置，用于描述基因组上的遗传距离和基因间的相对位置|
...
|第5列| 该SNP位点的等位基因情况在这n-5个样本中的分布情况|
|第n列|同第五列|
...
|第1列| 家系 ID，表示样本所属的家庭或群体|
|第2列| 个体 ID，表示样本的唯一标识符|
|第3列| 样本是否被包括在回归分析中。1表示该个体已被纳入回归分析；0表示该个体未被纳入回归分析|
|第4列| 个体多基因风险评分，用于预测个体患病的风险。通过加权求和计算得到的|
